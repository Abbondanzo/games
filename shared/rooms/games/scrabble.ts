...
export function decodeScrabbleAction(action: GameAction): Action | null {
  const result = ScrabbleActionSchema.safeParse(action);
  return result.success ? result.data : null;
}

export const scrabbleInitialState = (): Snapshot => initialState;

/**
 * Binds the room's own id source, so ids are minted once by the authority
 * rather than by whichever client happened to act.
 */
export function scrabbleApply(uid: IdSource): ApplyAction<Snapshot> {
  const reducer = createReducer(uid);
  return (snapshot, action) => {
    const state = GameStateSchema.safeParse(snapshot);
    const decoded = decodeScrabbleAction(action);
